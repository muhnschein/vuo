 ...
 * are traced here in JavaScript and the text is set along them by arc length.
 *
 * Two ways to leave room for whatever sits on top: a band at the top that the
 * text fades in beneath (`fadeFrom`..`fadeTo`, for a heading), and a disc it
 * fades out of (`clearRadius` around `clearX`,`clearY`, for a title in the
 * middle). Both off by default.
 *
 * The text is fixed and means nothing; nothing foreign is anywhere near this
 * file (§9.3).
 *
 * # It is drawn a slice at a time, and why
 *
 * Reported from a device: the onboarding page appeared and then sat at 100%
 * of a core for FOURTEEN SECONDS before its texture arrived. A page of this
 * at onboarding density is some tens of thousands of glyphs along a couple of
 * hundred thousand pixels of traced line, and the first version spent about
 * twenty times more work per pixel than it needed to. Four things were wrong,
 * and all four are fixed below:
 *
 *   - The gradient was taken by FINITE DIFFERENCES: four evaluations of the
 *     field for one gradient, and `settle` asked for three of those per step.
 *     Nineteen evaluations per two pixels. `fieldAt` now returns the value
 *     and the exact gradient from one pass over the strokes, and the walk
 *     needs two of those per step.
 *   - The step was two pixels everywhere. A curve at radius r deviates from
 *     its chord by about l²/8r, so the OUTER curves -- which are nearly all
 *     of the length -- can be walked in strides of twenty pixels and still
 *     sit within half a pixel of the truth. The step is now derived from
 *     that, which is where most of the tracing saving is.
 ...
 *     is set rather than stacked, and glyphs are drawn in RUNS of up to a
 *     few at a time -- a run only as long as the curve stays straight enough
 *     and the fade stays even enough for the difference to be invisible.
 *
 * Even so a whole page is not free, so the work is done in slices with a time
 * budget: each paint traces and draws as many rings as it can in `budgetMs`
 * and asks for another. Nothing ever blocks the UI for longer than that, and
 * the pattern grows outward from its centres while it works. `renderStrategy`
 * is deliberately `Immediate` -- the GUI thread -- because the slicing is
 * what keeps the app responsive, and a canvas painting on its own thread
 * cannot safely touch the Timer that drives it.
 */
Canvas {
    id: art
