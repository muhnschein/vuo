<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE TS>
<TS version="2.1" language="uk" sourcelanguage="en">
<context>
    <name>AddFeedPage</name>
    <message>
        <location filename="../qml/pages/AddFeedPage.qml" line="19"/>
        <source>Subscribe</source>
        <translation>Підписатися</translation>
    </message>
    <message>
        <location filename="../qml/pages/AddFeedPage.qml" line="23"/>
        <source>Feed or site address</source>
        <translation>Адреса стрічки або сайту</translation>
    </message>
    <message>
        <location filename="../qml/pages/AddFeedPage.qml" line="24"/>
        <source>https://example.com/feed.xml</source>
        <translation>https://pryklad.ua/feed.xml</translation>
    </message>
    <message>
        <location filename="../qml/pages/AddFeedPage.qml" line="42"/>
        <source>Your server discovers the feed and fetches it. Vuo never downloads feeds itself.</source>
        <translation>Ваш сервер знаходить стрічку й завантажує її. Vuo ніколи не завантажує стрічки сам.</translation>
    </message>
</context>
<context>
    <name>ArticlePage</name>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="136"/>
        <source>Article</source>
        <translation>Стаття</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="157"/>
        <source>Read</source>
        <translation>Прочитано</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="157"/>
        <source>Unread</source>
        <translation>Непрочитані</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="164"/>
        <source>★ Favourite</source>
        <translation>★ Обране</translation>
    </message>
    <message numerus="yes">
        <location filename="../qml/pages/ArticlePage.qml" line="179"/>
        <source>%n image(s) are not proxied by your server and were not loaded.</source>
        <translation>
            <numerusform>%n зображення не проходить через ваш сервер і не було завантажене.</numerusform>
            <numerusform>%n зображення не проходять через ваш сервер і не були завантажені.</numerusform>
            <numerusform>%n зображень не проходять через ваш сервер і не були завантажені.</numerusform>
        </translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="194"/>
        <source>This article was too large to display in full.</source>
        <translation>Стаття завелика, щоб показати її повністю.</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="199"/>
        <source>Mark as unread</source>
        <translation>Позначити непрочитаною</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="199"/>
        <source>Mark as read</source>
        <translation>Позначити прочитаною</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="208"/>
        <source>Remove favourite</source>
        <translation>Прибрати з обраного</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="209"/>
        <source>Add favourite</source>
        <translation>Додати до обраного</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="213"/>
        <source>Open in browser</source>
        <translation>Відкрити у браузері</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="225"/>
        <source>Fetch original content</source>
        <translation>Завантажити початкову статтю</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="233"/>
        <source>Asking the server for the original article…</source>
        <translation>Запитуємо в сервера початкову статтю…</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="390"/>
        <source>Tap to load images from %1</source>
        <translation>Торкніться, щоб завантажити зображення з %1</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="502"/>
        <source>Loaded the original article.</source>
        <translation>Початкову статтю завантажено.</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="506"/>
        <source>The server could not extract the original article.</source>
        <translation>Сервер не зміг видобути початкову статтю.</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="509"/>
        <source>This feed already carries the full article.</source>
        <translation>Ця стрічка вже містить статтю повністю.</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="511"/>
        <source>The server rejected the API key.</source>
        <translation>Сервер відхилив ключ API.</translation>
    </message>
    <message>
        <location filename="../qml/pages/ArticlePage.qml" line="515"/>
        <source>Could not fetch the original article: %1</source>
        <translation>Не вдалося завантажити початкову статтю: %1</translation>
    </message>
</context>
<context>
    <name>CoverPage</name>
    <message>
        <location filename="../qml/cover/CoverPage.qml" line="225"/>
        <source>Sign-in failed</source>
        <translation>Не вдалося увійти</translation>
    </message>
    <message>
        <location filename="../qml/cover/CoverPage.qml" line="225"/>
        <source>Refresh failed</source>
        <translation>Не вдалося оновити</translation>
    </message>
    <message>
        <location filename="../qml/cover/CoverPage.qml" line="226"/>
        <source>Refreshing</source>
        <translation>Оновлення</translation>
    </message>
</context>
<context>
    <name>EditFeedPage</name>
    <message>
        <location filename="../qml/pages/EditFeedPage.qml" line="100"/>
        <source>Could not save: %1</source>
        <translation>Не вдалося зберегти: %1</translation>
    </message>
    <message>
        <location filename="../qml/pages/EditFeedPage.qml" line="112"/>
        <source>Feed settings</source>
        <translation>Налаштування стрічки</translation>
    </message>
    <message>
        <location filename="../qml/pages/EditFeedPage.qml" line="117"/>
        <location filename="../qml/pages/EditFeedPage.qml" line="118"/>
        <source>Name</source>
        <translation>Назва</translation>
    </message>
    <message>
        <location filename="../qml/pages/EditFeedPage.qml" line="138"/>
        <source>Fetch original content</source>
        <translation>Завантажити початкову статтю</translation>
    </message>
    <message>
        <location filename="../qml/pages/EditFeedPage.qml" line="139"/>
        <source>The server scrapes each article&apos;s own page instead of using what the feed provides.</source>
        <translation>Сервер читає власну сторінку кожної статті замість того, що дає стрічка.</translation>
    </message>
    <message>
        <location filename="../qml/pages/EditFeedPage.qml" line="149"/>
        <source>Hide from unread</source>
        <translation>Сховати з непрочитаних</translation>
    </message>
    <message>
        <location filename="../qml/pages/EditFeedPage.qml" line="150"/>
        <source>Keep this feed&apos;s articles out of the Unread and All lists. The feed itself still shows them.</source>
        <translation>Не показувати статті цієї стрічки у списках «Непрочитані» та «Усі». Сама стрічка їх усе одно показує.</translation>
    </message>
    <message>
        <location filename="../qml/pages/EditFeedPage.qml" line="161"/>
        <source>Pause updates</source>
        <translation>Призупинити оновлення</translation>
    </message>
    <message>
        <location filename="../qml/pages/EditFeedPage.qml" line="162"/>
        <source>The server stops refreshing this feed.</source>
        <translation>Сервер припиняє оновлювати цю стрічку.</translation>
    </message>
</context>
<context>
    <name>EntryListPage</name>
    <message>
        <location filename="../qml/pages/EntryListPage.qml" line="11"/>
        <location filename="../qml/pages/EntryListPage.qml" line="167"/>
        <source>Unread</source>
        <translation>Непрочитані</translation>
    </message>
    <message>
        <location filename="../qml/pages/EntryListPage.qml" line="167"/>
        <source>Favourites</source>
        <translation>Обране</translation>
    </message>
    <message>
        <location filename="../qml/pages/EntryListPage.qml" line="167"/>
        <source>All</source>
        <translation>Усі</translation>
    </message>
    <message>
        <location filename="../qml/pages/EntryListPage.qml" line="317"/>
        <source>The server rejected the API key.</source>
        <translation>Сервер відхилив ключ API.</translation>
    </message>
    <message>
        <location filename="../qml/pages/EntryListPage.qml" line="318"/>
        <source>Open settings</source>
        <translation>Відкрити налаштування</translation>
    </message>
    <message>
        <location filename="../qml/pages/EntryListPage.qml" line="320"/>
        <source>Refresh failed: %1</source>
        <translation>Не вдалося оновити: %1</translation>
    </message>
</context>
<context>
    <name>EntryListView</name>
    <message numerus="yes">
        <location filename="../qml/components/EntryListView.qml" line="204"/>
        <source>%n selected</source>
        <translation>
            <numerusform>%n вибрана</numerusform>
            <numerusform>%n вибрані</numerusform>
            <numerusform>%n вибрано</numerusform>
        </translation>
    </message>
    <message numerus="yes">
        <location filename="../qml/components/EntryListView.qml" line="206"/>
        <source>%n article(s)</source>
        <translation>
            <numerusform>%n стаття</numerusform>
            <numerusform>%n статті</numerusform>
            <numerusform>%n статей</numerusform>
        </translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="261"/>
        <source>Select all</source>
        <translation>Вибрати всі</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="262"/>
        <source>Deselect all</source>
        <translation>Зняти вибір</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="267"/>
        <location filename="../qml/components/EntryListView.qml" line="619"/>
        <source>Mark as unread</source>
        <translation>Позначити непрочитаною</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="272"/>
        <location filename="../qml/components/EntryListView.qml" line="619"/>
        <source>Mark as read</source>
        <translation>Позначити прочитаною</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="279"/>
        <source>Settings</source>
        <translation>Налаштування</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="284"/>
        <source>Feeds</source>
        <translation>Стрічки</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="299"/>
        <location filename="../qml/components/EntryListView.qml" line="306"/>
        <source>Select articles</source>
        <translation>Вибрати статті</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="311"/>
        <source>Mark all as read</source>
        <translation>Позначити всі прочитаними</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="330"/>
        <source>Marking all as read</source>
        <translation>Позначаємо всі прочитаними</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="337"/>
        <source>Refresh</source>
        <translation>Оновити</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="381"/>
        <source>No favourites</source>
        <translation>Немає обраного</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="382"/>
        <source>Nothing to read</source>
        <translation>Немає чого читати</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="384"/>
        <source>Star an article to keep it here</source>
        <translation>Додайте статтю до обраного, щоб вона лишилася тут</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="385"/>
        <source>Pull down to refresh</source>
        <translation>Потягніть униз, щоб оновити</translation>
    </message>
    <message numerus="yes">
        <location filename="../qml/components/EntryListView.qml" line="540"/>
        <source>%n min read</source>
        <translation>
            <numerusform>%n хв читання</numerusform>
            <numerusform>%n хв читання</numerusform>
            <numerusform>%n хв читання</numerusform>
        </translation>
    </message>
    <message numerus="yes">
        <location filename="../qml/components/EntryListView.qml" line="541"/>
        <source>%n min</source>
        <translation>
            <numerusform>%n хв</numerusform>
            <numerusform>%n хв</numerusform>
            <numerusform>%n хв</numerusform>
        </translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="623"/>
        <source>Remove favourite</source>
        <translation>Прибрати з обраного</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="623"/>
        <source>Add favourite</source>
        <translation>Додати до обраного</translation>
    </message>
    <message>
        <location filename="../qml/components/EntryListView.qml" line="627"/>
        <source>Open in browser</source>
        <translation>Відкрити у браузері</translation>
    </message>
</context>
<context>
    <name>FeedListPage</name>
    <message>
        <location filename="../qml/pages/FeedListPage.qml" line="35"/>
        <source>Feeds</source>
        <translation>Стрічки</translation>
    </message>
    <message>
        <location filename="../qml/pages/FeedListPage.qml" line="39"/>
        <source>Add subscription</source>
        <translation>Додати підписку</translation>
    </message>
    <message>
        <location filename="../qml/pages/FeedListPage.qml" line="47"/>
        <source>No feeds</source>
        <translation>Немає стрічок</translation>
    </message>
    <message>
        <location filename="../qml/pages/FeedListPage.qml" line="48"/>
        <source>Subscribe from the pulley menu</source>
        <translation>Підпишіться через висувне меню</translation>
    </message>
    <message>
        <location filename="../qml/pages/FeedListPage.qml" line="146"/>
        <source>Feed</source>
        <translation>Стрічка</translation>
    </message>
    <message>
        <location filename="../qml/pages/FeedListPage.qml" line="154"/>
        <source>Feed settings</source>
        <translation>Налаштування стрічки</translation>
    </message>
    <message>
        <location filename="../qml/pages/FeedListPage.qml" line="169"/>
        <source>Mark feed as read</source>
        <translation>Позначити стрічку прочитаною</translation>
    </message>
    <message>
        <location filename="../qml/pages/FeedListPage.qml" line="173"/>
        <source>Unsubscribe</source>
        <translation>Відписатися</translation>
    </message>
    <message>
        <location filename="../qml/pages/FeedListPage.qml" line="174"/>
        <source>Unsubscribing</source>
        <translation>Відписуємось</translation>
    </message>
</context>
<context>
    <name>OnboardingPage</name>
    <message>
        <location filename="../qml/pages/OnboardingPage.qml" line="72"/>
        <source>Focus on what matters.</source>
        <translation>Зосередьтеся на головному.</translation>
    </message>
    <message>
        <location filename="../qml/pages/OnboardingPage.qml" line="82"/>
        <source>Continue</source>
        <translation>Далі</translation>
    </message>
</context>
<context>
    <name>SettingsPage</name>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="73"/>
        <source>Settings</source>
        <translation>Налаштування</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="75"/>
        <source>Account</source>
        <translation>Обліковий запис</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="80"/>
        <source>Server address</source>
        <translation>Адреса сервера</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="81"/>
        <source>https://miniflux.example.com</source>
        <translation>https://miniflux.pryklad.ua</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="89"/>
        <source>API key</source>
        <translation>Ключ API</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="106"/>
        <source>Create a key in Miniflux under Settings → API Keys.</source>
        <translation>Створіть ключ у Miniflux у розділі Settings → API Keys.</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="111"/>
        <source>Test connection</source>
        <translation>Перевірити з&apos;єднання</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="132"/>
        <source>Connected as %1</source>
        <translation>Під&apos;єднано як %1</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="133"/>
        <source>Test failed: %1</source>
        <translation>Перевірка не вдалася: %1</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="136"/>
        <source>Reading</source>
        <translation>Читання</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="141"/>
        <source>Mark as read when opened</source>
        <translation>Позначати прочитаною при відкритті</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="143"/>
        <source>Never</source>
        <translation>Ніколи</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="144"/>
        <source>Immediately</source>
        <translation>Одразу</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="145"/>
        <source>After 5 seconds</source>
        <translation>Через 5 секунд</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="146"/>
        <source>After 15 seconds</source>
        <translation>Через 15 секунд</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="147"/>
        <source>After 30 seconds</source>
        <translation>Через 30 секунд</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="159"/>
        <source>The delay counts only while the article is on screen. Marking an article read or unread yourself always wins.</source>
        <translation>Затримка триває, лише поки стаття на екрані. Позначка, яку ви поставили самі, завжди має перевагу.</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="162"/>
        <source>Images</source>
        <translation>Зображення</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="167"/>
        <source>Images not proxied by your server</source>
        <translation>Зображення не через ваш сервер</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="169"/>
        <source>Never load</source>
        <translation>Ніколи не завантажувати</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="170"/>
        <source>Ask each site</source>
        <translation>Питати для кожного сайту</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="171"/>
        <source>Always load</source>
        <translation>Завжди завантажувати</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="183"/>
        <source>Miniflux proxies plain-http images only by default, so most images arrive unproxied. Loading them directly tells those sites your IP address and when you read.</source>
        <translation>Типово Miniflux пропускає через себе лише зображення по звичайному HTTP, тож більшість надходить напряму. Завантаження напряму повідомляє тим сайтам вашу IP-адресу й час читання.</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="186"/>
        <source>Synchronisation</source>
        <translation>Синхронізація</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="194"/>
        <source>Sync with server</source>
        <translation>Синхронізація із сервером</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="196"/>
        <source>Manual only</source>
        <translation>Лише вручну</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="197"/>
        <source>Every 15 minutes</source>
        <translation>Кожні 15 хвилин</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="198"/>
        <source>Every 30 minutes</source>
        <translation>Кожні 30 хвилин</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="199"/>
        <source>Hourly</source>
        <translation>Щогодини</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="200"/>
        <source>Every 6 hours</source>
        <translation>Кожні 6 годин</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="214"/>
        <source>How often Vuo fetches from your Miniflux server while it is open or on the cover. How often the server itself checks your feeds is set on the server, not here.</source>
        <translation>Як часто Vuo звертається до вашого сервера Miniflux, поки відкритий або на обкладинці. Як часто сам сервер перевіряє стрічки, задається на сервері, а не тут.</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="223"/>
        <source>Only sync on Wi-Fi</source>
        <translation>Синхронізувати лише через Wi-Fi</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="224"/>
        <source>Vuo syncs on its own only over Wi-Fi, and waits on a mobile connection. A refresh you ask for yourself is always sent.</source>
        <translation>Сам по собі Vuo синхронізується лише через Wi-Fi, а на мобільному з&apos;єднанні чекає. Оновлення, яке ви попросили самі, надсилається завжди.</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="233"/>
        <source>Notify about new articles</source>
        <translation>Сповіщати про нові статті</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="234"/>
        <source>Shown when a sync finds new articles while Vuo is on the cover. Opening Vuo clears it.</source>
        <translation>Сповіщення з&apos;являється, коли синхронізація знаходить нові статті, поки Vuo на обкладинці. Відкриття Vuo прибирає його.</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="249"/>
        <source>Keep read articles</source>
        <translation>Зберігати прочитані статті</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="251"/>
        <source>Forever</source>
        <translation>Завжди</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="252"/>
        <source>For a month</source>
        <translation>Місяць</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="253"/>
        <source>For three months</source>
        <translation>Три місяці</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="254"/>
        <source>For six months</source>
        <translation>Шість місяців</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="255"/>
        <source>For a year</source>
        <translation>Рік</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="270"/>
        <source>Older articles you have already read are removed from this phone at the end of a sync. Favourites, unread articles and anything not yet sent to the server are always kept, and nothing is removed from your Miniflux server.</source>
        <translation>Старіші статті, які ви вже прочитали, вилучаються з цього телефона наприкінці синхронізації. Обране, непрочитані статті й усе, що ще не надіслано на сервер, зберігаються завжди, а з вашого сервера Miniflux не вилучається нічого.</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="281"/>
        <source>Advanced</source>
        <translation>Додатково</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="285"/>
        <source>Use a custom CA certificate</source>
        <translation>Власний сертифікат ЦС</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="292"/>
        <source>For a self-hosted server with a private certificate authority. Place the certificate at ~/.local/share/harbour-vuo/harbour-vuo/ca.pem. Certificate verification is never disabled, and there is no option to disable it.</source>
        <translation>Для власного сервера з приватним центром сертифікації. Покладіть сертифікат у ~/.local/share/harbour-vuo/harbour-vuo/ca.pem. Перевірка сертифікатів ніколи не вимикається, і такого налаштування немає.</translation>
    </message>
    <message>
        <location filename="../qml/pages/SettingsPage.qml" line="293"/>
        <source>Only applies to an https:// server. This one is not encrypted by TLS, so no certificate is used.</source>
        <translation>Стосується лише сервера https://. Цей не шифрується TLS, тож сертифікат не використовується.</translation>
    </message>
    <message numerus="yes">
        <location filename="../qml/pages/SettingsPage.qml" line="305"/>
        <source>%n change(s) waiting to be sent to the server.</source>
        <translation>
            <numerusform>%n зміна чекає на надсилання на сервер.</numerusform>
            <numerusform>%n зміни чекають на надсилання на сервер.</numerusform>
            <numerusform>%n змін чекають на надсилання на сервер.</numerusform>
        </translation>
    </message>
</context>
<context>
    <name>SetupDialog</name>
    <message>
        <location filename="../qml/pages/SetupDialog.qml" line="101"/>
        <source>Your Miniflux server</source>
        <translation>Ваш сервер Miniflux</translation>
    </message>
    <message>
        <location filename="../qml/pages/SetupDialog.qml" line="102"/>
        <source>Start reading</source>
        <translation>Почати читати</translation>
    </message>
    <message>
        <location filename="../qml/pages/SetupDialog.qml" line="112"/>
        <source>Vuo reads from your own Miniflux instance. It never fetches feeds itself.</source>
        <translation>Vuo читає з вашого власного сервера Miniflux. Стрічки він ніколи не завантажує сам.</translation>
    </message>
    <message>
        <location filename="../qml/pages/SetupDialog.qml" line="118"/>
        <source>Server address</source>
        <translation>Адреса сервера</translation>
    </message>
    <message>
        <location filename="../qml/pages/SetupDialog.qml" line="119"/>
        <source>https://miniflux.example.com</source>
        <translation>https://miniflux.pryklad.ua</translation>
    </message>
    <message>
        <location filename="../qml/pages/SetupDialog.qml" line="126"/>
        <source>API key</source>
        <translation>Ключ API</translation>
    </message>
    <message>
        <location filename="../qml/pages/SetupDialog.qml" line="142"/>
        <source>Create a key in Miniflux under Settings → API Keys.</source>
        <translation>Створіть ключ у Miniflux у розділі Settings → API Keys.</translation>
    </message>
    <message>
        <location filename="../qml/pages/SetupDialog.qml" line="151"/>
        <source>Test connection</source>
        <translation>Перевірити з&apos;єднання</translation>
    </message>
    <message>
        <location filename="../qml/pages/SetupDialog.qml" line="175"/>
        <source>Connected as %1</source>
        <translation>Під&apos;єднано як %1</translation>
    </message>
    <message>
        <location filename="../qml/pages/SetupDialog.qml" line="176"/>
        <source>Test failed: %1</source>
        <translation>Перевірка не вдалася: %1</translation>
    </message>
</context>
<context>
    <name>SitePage</name>
    <message>
        <location filename="../qml/pages/SitePage.qml" line="47"/>
        <location filename="../qml/pages/SitePage.qml" line="58"/>
        <source>Website</source>
        <translation>Сайт</translation>
    </message>
    <message>
        <location filename="../qml/pages/SitePage.qml" line="68"/>
        <source>Back</source>
        <translation>Назад</translation>
    </message>
    <message>
        <location filename="../qml/pages/SitePage.qml" line="73"/>
        <source>Open in browser</source>
        <translation>Відкрити у браузері</translation>
    </message>
</context>
<context>
    <name>harbour-vuo</name>
    <message numerus="yes">
        <location filename="../qml/harbour-vuo.qml" line="125"/>
        <source>%n new article(s)</source>
        <translation>
            <numerusform>%n нова стаття</numerusform>
            <numerusform>%n нові статті</numerusform>
            <numerusform>%n нових статей</numerusform>
        </translation>
    </message>
</context>
</TS>
