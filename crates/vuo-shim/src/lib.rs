...
//! Almost all of it is generated by the `QObject` derive and the `cpp!` macro.
//! There is exactly ONE hand-written `unsafe` block, and it is worth naming
//! here so that a second one has to be argued for: [`memory`] calls glibc's
//! `malloc_trim` to give the sync's freed peak back to the kernel. It is
//! reviewed in place.
//!
